    ...
    public class Notifier implements Runnable {
        ...
        private void handle(Pair<String, DataOperation> pair) {
            try {
                ...
                // 通过键（datumKey）从服务列表（services）中移除对应的服务。
                services.remove(datumKey);
                int count = 0;
                // 检查是否存在监听器（listeners）注册到该键（datumKey），如果不存在，则直接返回，不进行后续的通知操作。
                if (!listeners.containsKey(datumKey)) {
                    return;
                }
                // 遍历该键（datumKey）对应的所有监听器（listeners.get(datumKey)），对每个监听器进行相应的通知操作。
                for (RecordListener listener : listeners.get(datumKey)) {
                    count++;
                    try {
                        // 需要进行变更（CHANGE），需要的执行操作
                        // 如果操作（action）是 DataOperation.CHANGE，调用监听器的 onChange 方法，并传递数据的键和值。
                        if (action == DataOperation.CHANGE) {
                ...
                // 处理 debug日志，打印主要信息 key和次数 操作的名称 action.name
